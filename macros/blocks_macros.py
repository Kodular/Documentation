...
class PropertyBlockGetAndSetMacro(mdx_macros.BaseMacro):
    ...
    def handler(self, component_name, property_name, *args, **kwargs):
        ...
        # Extra <div> has been added to maintain a correct XML structure, because the markdown-macros parses the
        # returned string as XML
        return '<div>' + setter + '\n' + getter + '</div>'
...
class MethodBlockMacro(mdx_macros.BaseMacro):
    ...
    Usage:
        ...
        - `[[Method('Device Utility', 'Copy', False, 'success Toast Message')]]` - for a  method with no return value
            and with parameters.
        - `[[Method('Color Utility', 'Get Luminance', True,  'color')]]` - for a method with return value and with
            parameters.
